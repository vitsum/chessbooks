...
# ================= пояснения от ИИ =================
# Книга объясняет далеко не каждый ход. Недостающее можно дописать здесь, но такие
# пояснения обязаны быть видимо отделены от авторских: они не проверены ни автором,
# ни редактором и могут быть просто неверны. Поэтому текст приходит сюда голым,
# а обёртку с пометкой ставит сборщик — забыть её нельзя.
#
#   AI_COMMON = {"3.Nc3": "…"}          — на ход в любой книге, где он есть
#   AI = {"id": {"7...Nc6": "…"}}       — на ход в конкретном варианте
#
# Ставится ПОСЛЕ разбора кликабельных ходов: в пояснениях ИИ нарочно нет записи
# ходов, и превращать их в ссылки не нужно.
AI        = globals().get("AI", {})
AI_COMMON = globals().get("AI_COMMON", {})
_ai_n = 0
for v in V:
    own = AI.get(v["id"], {})
    for lab in own:                       # опечатка в метке иначе молча ничего не сделает
        if lab not in v["labels"]:
            raise SystemExit(f"[AI/{v['id']}] нет хода {lab}\n  есть: {v['labels']}")
    for i, lab in enumerate(v["labels"]):
        k = str(i + 1)
        if k in v["notes"]: continue      # автор этот ход уже объяснил — не мешаем
        txt = own.get(lab) or AI_COMMON.get(lab)
        if not txt: continue
        v["notes"][k] = f'<span class="ai">{txt}</span>'
        _ai_n += 1
if _ai_n: print("пояснений ИИ:", _ai_n)
